  ...
  Game.prototype.addEvent = function(event){
    ...
    if(this.projectedState != null){
      if(event.vt >= this.projectedState.vt){
        ...
        // sync clocks with last observed server clock (minus 250 milliseconds to decrease likelihood of missed events)
        this.projectedState.clock = this.state.clock = event.vt - window.Utilities.ms2ticks(250);
      }
      else{
        var msGap = window.Utilities.ticks2ms(this.projectedState.vt - event.vt);
        console.log('invalidating... missed it by ' + msGap + ' ms');
