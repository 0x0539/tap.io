window.Game = (function(){
  ...
  var Game = function(socket){
    ...
    // send the event back for latency estimation
    this.socket.on('ping', function(event){ dis.emit('pong', event); });

    // these are all timeline events
    this.socket.on('startSession', function(event){ dis.addEvent(event); });
    this.socket.on('endSession', function(event){ dis.addEvent(event); });
    this.socket.on('gameevent', function(event){ dis.addEvent(event); });
    this.socket.on('heartbeat', function(event){ dis.addEvent(event); });
  };
