var Game = function(network, state){
  ...
  if(typeof this.state.clock != 'number')
    throw new Error('state clock is not a number');

  if(typeof this.state.vt != 'number')
    throw new Error('state vt is not a number');

  if(typeof this.state.events != 'object')
    throw new Error('state events array is not an object');

  if(typeof this.state.sessionIds != 'object')
    throw new Error('state sessionIds array is not an object');
  ...
  for(var i = 0; i < this.state.sessionIds.length; i++){
    ...
    if(sessionId != 0){
      var event = this.buildEvent('disconnect', 0, {sessionId: sessionId});
  ...
  this.network.on('connect', function(sessionId){
    dis.fire(dis.buildEvent('connect', 0, {sessionId: sessionId}));
  ...
  this.network.on('disconnect', function(sessionId){
    dis.fire(dis.buildEvent('disconnect', 0, {sessionId: sessionId}));
  });
